 ...
 */
function governlia_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'governlia_unset_default_widgets_args' );
 ...
 */
function governlia_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'governlia_child_functions_php', 10, 2 );
 ...
 */
function governlia_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'governlia'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'governlia_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'governlia' ),
			'preview_url'              => 'http://fastwpdemo.com/newwp/governlia/',
		),
	);
}

add_filter( 'merlin_import_files', 'governlia_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function governlia_after_import_setup() {
...
add_action( 'merlin_after_all_import', 'governlia_after_import_setup' );