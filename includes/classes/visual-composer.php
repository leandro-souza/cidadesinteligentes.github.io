...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'governlia_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wpgovernlia_shortcode' ) ) {
					$tag = esc_attr( governlia_set( $data, 'base' ) );

					wpgovernlia_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		governlia_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = governlia_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "governlia_vc_map_file_{$tag}", $file );
	...
	function shortcodeParams( $tag ) {
		...
		if ( file_exists( $file ) ) {
			...
			return governlia_set( $data, 'params' );
		}
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'governlia' ) => 'true' );
		if ( ! empty( $values ) ) {
